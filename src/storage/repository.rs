...
impl SqliteRepository {
    ...
    #[tokio::test]
    async fn owner_domain_delivery_methods_are_consistent() {
        let path = std::env::temp_dir().join(format!(
            "guigu-owner-domain-delivery-{}.db",
            uuid::Uuid::now_v7()
        ));
        let owner = BusinessStoreOwner::open(&path).expect("owner");
        owner.execute(|connection| connection.execute_batch("CREATE TABLE deliveries(delivery_id TEXT PRIMARY KEY, task_id TEXT NOT NULL, attempt INTEGER NOT NULL, target_endpoint_id TEXT NOT NULL, dispatched_at TEXT NOT NULL, acknowledged_at TEXT); CREATE TABLE task_events(event_id TEXT PRIMARY KEY, task_id TEXT NOT NULL, seq INTEGER NOT NULL, status TEXT NOT NULL, timestamp TEXT NOT NULL, payload TEXT NOT NULL)").map_err(|error| StorageError::OwnerQuery(error.to_string()))).expect("schema");
