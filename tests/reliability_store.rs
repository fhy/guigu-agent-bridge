...
#[tokio::test]
async fn queue_pause_rejects_forged_reap_binding_and_recovery_classifies_send_state() {
    let pool = database().await;
    let (endpoint, _conversation, task, _event) = seed(&pool, "queued").await;
    let delivery = Uuid::now_v7().to_string();
    sqlx::query("INSERT INTO deliveries(delivery_id,task_id,attempt,target_endpoint_id,dispatched_at,acknowledged_at) VALUES(?,?,1,?,?,NULL)")
    ...
    let reservation = store
        ...
        .await
        .unwrap();
    sqlx::query("UPDATE agent_work_queue SET state='running',runtime_owner='owner-a',owner_fence=7,send_started=1 WHERE queue_id=?")
